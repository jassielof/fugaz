...
pub fn close(self: *TempPath) !void {
    if (self.path_buf.len == 0) {
        return;
    }

    if (!self.retain) {
        try util.deleteAbsolute(self.kind, self.path_buf);
    }
...
pub fn persist(self: *TempPath, new_path: []const u8) !void {
    if (self.path_buf.len == 0) {
        return;
    }

    try util.renameAbsolute(self.path_buf, new_path);
    self.releaseMemory();
}

pub fn deinit(self: *TempPath) void {
    if (self.path_buf.len != 0 and !self.retain) {
        util.deleteAbsolute(self.kind, self.path_buf) catch {};
    }
    self.releaseMemory();
}
...
test TempPath {
    ...
    var dir = try std.fs.openDirAbsolute(parent_path, .{});
    defer dir.close();
    const file = try dir.createFile("owned.txt", .{});
    file.close();

    var temp_path = TempPath.init(allocator, owned_path, .file, false);
    try temp_path.close();

    try std.testing.expectError(error.FileNotFound, dir.openFile("owned.txt", .{}));
}
