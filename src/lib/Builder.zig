...
pub fn tempDir(self: Builder, io: Io, allocator: Allocator) !TempDir {
...
pub fn tempFile(self: Builder, io: Io, allocator: Allocator) !TempFile {
    const parent_path = try env.tempDirPathAlloc(allocator);
    defer allocator.free(parent_path);
...
test Builder {
    ...
    var temp_file = try builder.tempFile(io, allocator);
